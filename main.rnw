%% Time-stamp: <2016-07-16 11:49:53 chl>
...
f(pvals)
@ 
Alternatively, one can dig into the \texttt{multtest} package
(\url{htpp://www.bioconductor.org}) which features the \texttt{mt.rawp2adjp()}
command. 
